    ...
    <div className="flex flex-col gap-4 p-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">Deployment in {namespace} - Ready: {ready}</p>
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-3">
              ...
              <div className="rounded-md border p-3 space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Updated</span><span>{status.updatedReplicas || 0}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Available</span><span>{status.availableReplicas || 0}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Strategy</span><span>{spec.strategy?.type || '-'}</span></div>
                  ...
                  <tbody>
                    {(status.conditions || []).map((c: any, i: number) => (
                      <tr key={i} className="border-t">
                        ...
                        <td className="px-3 py-2 text-muted-foreground text-xs">{c.message || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <ConfirmDialog
        ...
        title={`Delete ${name}?`}
        description={`This will permanently delete the deployment "${name}". This action cannot be undone.`}
        confirmLabel="Delete"
        variant="destructive"
